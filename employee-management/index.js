...
async function mainMenu() {
  console.log('Main menu is being called');
  ...
  switch (action) {
    ...
    case 'Update an employee role':
      const rolesForUpdate = await getAllRoles(); // Initialize rolesForUpdate here
      const employeesForUpdate = await getAllEmployees();
      const roleChoicesForUpdate = rolesForUpdate.map(role => ({ name: role.title, value: role.id })); // Now reference rolesForUpdate
